...
print(f"C-weight: 2 stages x [1,-2,1] = 4 zeros at DC (should be 2)")
...
print(f"// A-weight stage 1: LP 107.7+737.9 Hz poles, NO zeros at DC")
print(f"  {{{b1[0]:.15e}f,{b1[1]:.15e}f,{b1[2]:.15e}f,  {a1[0]:.10f}f,{a1[1]:.10f}f, 0,0}},")
print(f"// A-weight stage 2: HP 12194 Hz double pole + 2 zeros at DC")
...
print(f"// C-weight stage 1: LP 12194 Hz double pole, NO zeros at DC")
...
iec_a = {31.5:-39.4, 63:-26.2, 125:-16.1, 250:-8.6, 500:-3.2, 1000:0.0, 2000:1.2, 4000:1.0, 8000:-1.1, 16000:-6.6}
...
print(f"{'freq':>8} {'computed':>10} {'IEC':>10} {'err':>8}")
iec_c = {31.5:-3.0, 63:-0.8, 125:-0.2, 250:0.0, 500:0.0, 1000:0.0, 2000:-0.2, 4000:-0.8, 8000:-3.0, 16000:-8.5}
