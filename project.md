...
**Output:** Integrated 8x12 LED Matrix

```text
mobile_spl/
├── app.yaml            # Uno Q app manifest (MPU side)
├── python/
│   ├── main.py         # Python entry point (MPU — stub for Phase 1)
│   └── requirements.txt
├── sketch/
│   ├── sketch.ino      # Arduino sketch (MCU)
│   └── sketch.yaml     # Board/library declarations
└── .vscode/
    ├── settings.json   # UNO Q IP address — update here
    └── tasks.json      # Deploy / Start / Stop / Logs tasks
```
...
* **Weighting Filters:**
  * **dBA:** For environmental/human hearing noise.
...
* **Joystick UP / DOWN:** Cycle Weighting Mode (A → C).
...
3. **DSP Pipeline:** Remove DC offset → apply IIR filter (A/B/C coefficients) → calculate RMS: $V_{rms} = \sqrt{\frac{1}{n} \sum V^2}$.
...
* [ ] **Library:** `Arduino_LED_Matrix` (declared in `sketch/sketch.yaml`)
...
* [ ] **Deploy:** `Ctrl+Shift+B` → *Uno Q: Deploy & Start* (SCP + `arduino-app-cli app start` on board).
